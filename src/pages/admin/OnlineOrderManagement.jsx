...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap ${s.bg} ${s.text} ${s.border}`}
    >
...
export default function OrderManagement() {
  ...
  const stats = [
    ...
    {
      label: "Revenue",
      value: `Rs ${orders.reduce((s, o) => s + parseFloat(o.total_amount ?? 0), 0).toFixed(0)}`,
      color: "#5a46c2",
    },
  ];
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-5 sm:py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-5 sm:mb-6">
          {stats.map((s, i) => (
            ...
            >
              ...
              <span
                className="text-2xl sm:text-[1.6rem] font-bold leading-none mt-1"
                style={{ color: s.color }}
              >
                {s.value}
              </span>
        ...
        <div className="bg-white border border-[#e5e0ff] rounded-2xl shadow-sm overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-[#f0eeff]">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative flex-1">
                <FiSearch
                  className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9090b0]"
                  size={14}
                />
                <input
              ...
              <thead className="bg-[#f9f8ff] border-b-2 border-[#eeeaff]">
                <tr>
                  {COLUMNS.map((col) => (
                    <th
                      key={col}
                      className="px-5 py-3.5 text-[0.7rem] font-bold text-[#6d6a8a] uppercase tracking-wider whitespace-nowrap"
                    >
                      {col}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-[#f0eeff]">
                {paginated.length === 0 ? (
                  <tr>
                    <td
                      colSpan={8}
                      className="text-center py-16 text-[#b0add0] text-sm"
                    >
                      No orders found.
                    </td>
                  </tr>
                  ...
                  paginated.map((order) => {
                    ...
                    return (
                      <tr
                        key={order.order_id}
                        className="hover:bg-field-bg transition-colors cursor-pointer"
                      >
                        <td className="px-5 py-4">
                          ...
                          {order.items?.length > 1 && (
                            <p className="text-[10px] text-[#b0add0] mt-0.5">
                        ...
                        <td className="px-5 py-4">
                          ...
                          <p className="font-mono text-[10px] text-[#b0add0] mt-0.5">
                            {order.shippingAddress?.cus_phone}
                          </p>
                        </td>
                        <td className="px-5 py-4">
          ...
          <div className="md:hidden divide-y divide-[#f0eeff]">
            {paginated.length === 0 ? (
              <div className="text-center py-12 text-[#b0add0] text-sm">
                No orders found.
              </div>
            ) : (
              paginated.map((order) => {
                const productName = order.items?.[0]?.product?.p_name ?? "—";
                const customerName =
                  order.shippingAddress?.cus_name ?? order.user?.email ?? "—";
                const totalQty =
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="min-w-0">
                        <p className="font-semibold text-[#1e1b4b] text-sm truncate mt-0.5">
                          {productName}
                        </p>
                      </div>
                      <StatusBadge status={order.status} />
                    ...
                    <div className="grid grid-cols-3 gap-2 my-3">
                      <div className="bg-[#f9f8ff] border border-[#f0eeff] rounded-xl px-2.5 py-2">
                        <p className="text-[9px] font-bold uppercase tracking-wider text-[#b0add0] mb-0.5">
                          Customer
                        </p>
                      ...
                      <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-2.5 py-2">
                        <p className="text-[9px] font-bold uppercase tracking-wider text-indigo-300 mb-0.5">
                          Total
                        </p>
