...
export default function OrderManagement() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const onlineOrders = res.data.filter((o) => o.payment_method === "ONLINE");        
  ...
  const stats = [
    ...
    { label: "Revenue",      value: `Rs ${orders.reduce((s, o) => s + parseFloat(o.total_amount ?? 0), 0).toFixed(0)}`, color: "#5a46c2" },
  ];
            ...
            <table className="w-full text-sm text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#f0eeff]">
                {paginated.length === 0 ? (
                  ...
                  const customerName = order.shippingAddress?.cus_name ?? order.user?.email ?? "—";
                  ...
                  return (
                    <tr key={order.order_id} className="hover:bg-field-bg transition-colors cursor-pointer">
                      <td className="px-5 py-4">
                        <p className="font-semibold text-[#1e1b4b] text-sm truncate max-w-40">{productName}</p>
                        {order.items?.length > 1 && (
                          <p className="text-[10px] text-[#b0add0] mt-0.5">+{order.items.length - 1} more</p>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <p className="font-semibold text-[#1e1b4b] text-sm">{customerName}</p>
                        <p className="font-mono text-[10px] text-[#b0add0] mt-0.5">{order.shippingAddress?.cus_phone}</p>
                      </td>
                      <td className="px-5 py-4">
                        <span className="font-semibold text-[#1e1b4b]">{totalQty}</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="font-mono font-bold text-[#1e1b4b]">Rs {parseFloat(order.total_amount).toFixed(2)}</span>
                      </td>
                      <td className="px-5 py-4">
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-[#1e1b4b] text-sm truncate mt-0.5">{productName}</p>
                    </div>
                    <StatusBadge status={order.status} />
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 my-3">
                    <div className="bg-[#f9f8ff] border border-[#f0eeff] rounded-xl px-2.5 py-2">
                      ...
                      <p className="text-xs font-semibold text-[#1e1b4b] truncate">{customerName.split(" ")[0]}</p>
                    </div>
                    <div className="bg-[#f9f8ff] border border-[#f0eeff] rounded-xl px-2.5 py-2">
                      <p className="text-[9px] font-bold uppercase tracking-wider text-[#b0add0] mb-0.5">Qty</p>
                      <p className="text-xs font-semibold text-[#1e1b4b]">{totalQty} units</p>
                    </div>
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-2.5 py-2">
                      ...
                      <p className="text-xs font-bold text-[#5a46c2] font-mono">Rs {parseFloat(order.total_amount).toFixed(2)}</p>
                    </div>
                  </div>
