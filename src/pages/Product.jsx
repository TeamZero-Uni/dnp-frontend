...
function Product() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesStock =
      !inStockOnly || product.status === "stock" || product.p_status === "IN_STOCK";
    ...
    <>
      ...
      <div className="container max-w-7xl mx-auto px-4 py-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* --- SIDEBAR --- */}
          {/* Mobile-only backdrop to close the drawer when users tap outside the filter panel. */}
          {isFilterOpen && (
            <div
          ...
          <aside
            className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-white lg:bg-transparent overflow-y-auto lg:overflow-visible transition-transform duration-300 ease-in-out lg:static lg:block lg:w-[280px] lg:z-auto lg:translate-x-0 ${
          ...
          >
            <div className="flex items-center justify-between p-5 lg:hidden border-b border-gray-100 bg-white sticky top-0 z-10">
              <span className="font-black text-lg text-[#06021d] uppercase tracking-wider">
                Filters
              </span>
              <button
                onClick={() => setIsFilterOpen(false)}
                className="p-2 text-gray-400 hover:text-rose-500 bg-gray-50 rounded-full transition-colors"
              >
                <FaTimes />
              </button>
            </div>
            ...
            <div className="p-5 lg:p-0">
              <div className="bg-white p-5 rounded-2xl shadow-md border border-gray-100">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <div
                        ...
                        className={`w-12 h-6 rounded-full transition-colors duration-300 relative cursor-pointer ${
                          inStockOnly ? "bg-blue-500" : "bg-gray-200"
                        }`}
                      >
                        <span
                ...
                >
                  {/* Two range inputs are layered to create a single draggable min/max price slider. */}
          ...
          <main className="flex-1 min-w-0 relative z-0">
            ...
            <div className="flex flex-wrap items-center justify-between mb-8 gap-4">
              <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight hidden lg:block">
                Products
              </h2>

              <button
                onClick={() => setIsFilterOpen(true)}
                className="lg:hidden flex items-center gap-2 bg-white px-5 py-2.5 rounded-xl shadow-sm border border-slate-100 text-sm font-black text-[#5a46c2] hover:bg-slate-50 transition-colors"
              >
                <FaFilter /> FILTERS
              </button>

              {/* SORT BAR */}
              <div className="relative z-20 min-w-[220px] max-w-full ml-auto lg:ml-0">
                <select
                  value={sortBy}
            ...
            >
              {/* Animate each card so the list reflows smoothly when filters or sorting change. */}
            ...
            {filteredProducts.length === 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300 mt-5"
              >
