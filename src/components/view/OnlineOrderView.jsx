...
import { Package, Phone, MapPin, CreditCard, Calendar, User, Hash, ChevronDown, Truck } from 'lucide-react';
import {  changeOrderStatus, updatePaymentStatus, updateTrackingNumber } from '../../api/ordersApi';
...
export default function OnlineOrderView({ order }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await changeOrderStatus({id: order.order_id, status: status});
      if (status === "SHIPPED" && trackingInput) {
        await updateTrackingNumber({id: order.order_id, tracking_number: trackingInput});
      ...
      <div className="flex flex-col gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
        ...
        <div className="flex flex-wrap gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-gray-800 font-mono">{order.order_id.slice(0, 13)}…</p>
            </div>
          </div>
        ...
        {status === "SHIPPED" && (
          <div className="flex flex-wrap items-center gap-2 bg-purple-50 border border-purple-200 rounded-xl px-3 py-2.5">
            ...
            <span className="text-xs font-bold text-purple-700 whitespace-nowrap">Tracking No.</span>
            <input
              type="text"
              value={trackingInput}
            ...
            />
            {savedTracking && (
              <span className="text-[10px] text-purple-400 italic whitespace-nowrap">Saved: {savedTracking}</span>
            )}
          </div>
        )}
      </div>
        ...
        <div className="space-y-5">
          ...
          <section>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 space-y-2">
              <p className="text-sm font-semibold text-gray-800">{shipping.cus_name ?? order.user?.email}</p>
          ...
          <section>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 text-sm text-gray-600 leading-relaxed">
              ...
              <p>{shipping.cus_city}{shipping.cus_state ? `, ${shipping.cus_state}` : ""}</p>
              {shipping.cus_zip && <p>{shipping.cus_zip}</p>}
              <p className="font-medium text-gray-400 mt-1">{shipping.cus_country}</p>
            </div>
          </section>
            ...
            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 flex justify-between items-center flex-wrap gap-2">
              <div>
                ...
                {savedTracking && status === "SHIPPED" && (
                  <p className="text-[10px] text-indigo-400 mt-0.5">Tracking: {savedTracking}</p>
                )}
              </div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${
                paymentStatusStyles[order.payment_status] ?? "bg-gray-100 text-gray-700 border-gray-200"
              }`}>
          ...
          <section>
            ...
            <div className="bg-white rounded-xl border border-gray-100 flex flex-col overflow-hidden">
              ...
              <div className="overflow-y-auto max-h-64">
                {items.length === 0 ? (
                  ...
                  <div key={idx} className="flex justify-between items-center p-4 border-b border-gray-50 last:border-0">
                    <div>
                      <p className="text-sm font-bold text-gray-800">{item.product?.p_name ?? "Product"}</p>
                      <p className="text-xs text-gray-500 font-medium mt-0.5">Qty: {item.quantity}</p>
                    </div>
                    <p className="text-sm font-bold text-indigo-600">
                      Rs {(parseFloat(item.unit_price ?? 0) * (item.quantity ?? 1)).toFixed(2)}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-between items-center px-4 py-3 bg-indigo-50/60 border-t border-indigo-100 shrink-0">
                <p className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Total</p>
                <p className="text-base font-bold text-indigo-700">Rs {parseFloat(order.total_amount).toFixed(2)}</p>
              </div>
            </div>
          </section>
        </div>
