...
import { Package, Phone, MapPin, CreditCard, Calendar, User, Hash, ChevronDown, Banknote } from 'lucide-react';

export default function CodOrderView({ order }) {
  ...
  const handleCollectMoney = () => {
    ...
    alert(`Payment of $${order.total_amount.toFixed(2)} recorded successfully!`);
  };
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 space-y-4">
              <div className="flex justify-between items-center">
                <p className="text-xs text-indigo-700 font-bold uppercase">{order.payment_method}</p>
              ...
              {order.payment_status !== 'paid' && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 btn-color rounded-lg text-xs font-bold transition-all shadow-md shadow-indigo-200"
                >
                  <Banknote size={16} />
                  Collect ${order.total_amount.toFixed(2)} Cash
                </button>
              )}
            </div>
          </section>
